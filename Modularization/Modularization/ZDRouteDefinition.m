...
@implementation ZDRouteDefinition
...
- (BOOL)isEqual:(id)object
{
    ...
    if ([object isKindOfClass:[ZDRouteDefinition class]]) {
        return [self isEqualToRouteDefinition:(ZDRouteDefinition *)object];
    } else {
...
- (ZDRouteResponse *)routeResponseForRequest:(ZDRouteRequest *)request
{
    ...
    if (request.pathComponents.count != self.patternPathComponents.count && !patternContainsWildcard) {
        ...
        return [ZDRouteResponse invalidMatchResponse];
    }
    ...
    if (routeVariables != nil) {
        ...
        return [ZDRouteResponse validMatchResponseWithParameters:matchParams];
    } else {
...
- (NSDictionary <NSString *, NSString *> *)routeVariablesForRequest:(ZDRouteRequest *)request
{
    ...
    for (NSString *patternComponent in self.patternPathComponents) {
        ...
        if ([patternComponent hasPrefix:@":"]) {
            ...
            BOOL decodePlusSymbols = ((request.options & ZDRouteRequestOptionDecodePlusSymbols) == ZDRouteRequestOptionDecodePlusSymbols);
            variableValue = [ZDParsingUtilities variableValueFrom:variableValue decodePlusSymbols:decodePlusSymbols];
            ...
            if (request.pathComponents.count >= minRequiredParams) {
                ...
                routeVariables[ZDRouteWildcardComponentsKey] = [request.pathComponents subarrayWithRange:NSMakeRange(index, request.pathComponents.count - index)];
                isMatch = YES;
            } else {
...
- (NSDictionary *)matchParametersForRequest:(ZDRouteRequest *)request routeVariables:(NSDictionary <NSString *, NSString *> *)routeVariables
{
    ...
    BOOL decodePlusSymbols = ((request.options & ZDRouteRequestOptionDecodePlusSymbols) == ZDRouteRequestOptionDecodePlusSymbols);
    [matchParams addEntriesFromDictionary:[ZDParsingUtilities queryParams:request.queryParams decodePlusSymbols:decodePlusSymbols]];
...
{
    return @{ZDRoutePatternKey: self.pattern ?: [NSNull null], ZDRouteURLKey: request.URL ?: [NSNull null], ZDRouteSchemeKey: self.scheme ?: [NSNull null]};
}
...
{
    ZDRouteDefinition *copy = [[[self class] alloc] initWithPattern:self.pattern priority:self.priority handlerBlock:self.handlerBlock];
